...
export default function Settings({ onShowToast }) {
  ...
  const [accountStatus, setAccountStatus] = useState('Connected');
  ...
  const handleCopyRole = () => {
    navigator.clipboard.writeText(accountDetails.iamRoleArn);
    setCopiedRole(true);
      ...
      {error ? (
        ...
        <div className="space-y-3">
          <div className="text-xs font-semibold text-primary animate-pulse flex items-center gap-2">
            ...
            Loading account governance settings...
          </div>
          ...
          <div className="glass-card p-6 sm:p-8 rounded-3xl border border-outline-variant/15 space-y-6">
            <div className="flex items-center justify-between border-b border-outline-variant/15 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-on-surface">{accountDetails.accountName}</h2>
                  <span className="text-xs text-on-surface-variant/70">AWS Account ID: {accountDetails.accountId}</span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-surface-container-low p-4 rounded-2xl border border-outline-variant/15">
              <div>
                ...
                <div className="flex items-center justify-between bg-white p-2.5 rounded-xl border border-outline-variant/20">
                  <span className="text-xs text-on-surface font-mono truncate mr-2">{accountDetails.iamRoleArn}</span>
                  <button
              ...
              <div className="flex flex-col justify-between">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {accountDetails.regionsMonitored.map((r, i) => (
            ...
            <div className="flex items-center justify-between pt-2">
              <span className="text-xs text-on-surface-variant/70">Collector engine: {accountDetails.collectorVersion}</span>
              <div className="flex items-center gap-3">
                <button
                  onClick={() => {
                    if (onShowToast) {
